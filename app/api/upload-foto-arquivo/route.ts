...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`📸 Foto enviada otimizada: ${imageBuffer.length} bytes → ${optimizedBuffer.length} bytes (${Math.round((1 - optimizedBuffer.length/imageBuffer.length) * 100)}% menor)`);
